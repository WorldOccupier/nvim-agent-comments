...
local function comment_lines(comment, status)
  local width = math.max(BOX_MIN_WIDTH, vim.o.columns - BOX_MARGIN)
  ...
  local label = status == 'stale' and 'Stale comment' or 'Agent comment'
  local top = '┌ ' .. label .. ' [x] '
  local top_border = string.rep(BOX_BORDER, math.max(1, width - vim.fn.strdisplaywidth(top)))
  local bottom = '└' .. string.rep(BOX_BORDER, width - 1)
  return {
    { { top, group }, { top_border, border_group } },
    { { '│ ' .. comment.body, 'CommentBoxText' } },
    { { bottom, border_group } },
  }
end
...
function M.render(bufnr)
  ...
  for _, comment in ipairs(comments.comments) do
    if comment.path == relative then
      ...
      local display_line = resolved.end_line or comment.end_line
      if display_line >= 1 and display_line <= #lines then
        vim.api.nvim_buf_set_extmark(bufnr, NAMESPACE, display_line - 1, 0, {
          virt_lines = comment_lines(comment, status),
          virt_lines_above = false,
          virt_lines_leftcol = false,
        })
        if M.config.signs then
          vim.api.nvim_buf_set_extmark(bufnr, NAMESPACE, (resolved.start_line or comment.start_line) - 1, 0, {
            sign_text = status == 'stale' and STALE_SIGN or SIGN,
